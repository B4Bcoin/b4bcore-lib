...
async.series([
  function(next) {
    ...
    for (var i = 0; i < 100; i++) {
      ...
      // uint64le
      var br = new b4bcore.encoding.BufferWriter();
      var num = Math.round(Math.random() * 10000000000000);
      br.writeUInt64LEBN(new b4bcore.crypto.BN(num));
      buffers.push(br.toBuffer());

      // hashes
      var data = b4bcore.crypto.Hash.sha256sha256(new Buffer(32));
    ...
    function readUInt64LEBN() {
      ...
      var buf = buffers[c];
      var br = new b4bcore.encoding.BufferReader(buf);
  ...
  function(next) {
    ...
    function b4bcoreTest() {
      block1 = b4bcore.Block.fromString(blockData);
    }
    ...
    var suite = new benchmark.Suite();
    suite.add('b4bcore', b4bcoreTest, {maxTime: maxTime});
