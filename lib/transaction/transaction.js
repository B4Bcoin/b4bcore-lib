...
/**
 * Retrieve a hexa string that can be used with b4bd's CLI interface
 ...
 * * `disableMoreOutputThanInput`: disable checking if the transaction spends more b4bcoins than the sum of the input amounts
 * @return {string}
 */
 ...
 *
 * Can receive, as output information, the output of b4bd's `listunspent` command,
 * and a slightly fancier format recognized by b4bcore:
 *
 * ```
 * {
 ...
 * }
 * ```
 * Where `address` can be either a string or a b4bcore Address object. The
 * same is true for `script`, which can be a string or a b4bcore Script.
 *
 ...
 *
 * // From a pay to public key hash output from b4bd's listunspent
...
/**
 * Analogous to b4bd's IsCoinBase function in transaction.h
 */
